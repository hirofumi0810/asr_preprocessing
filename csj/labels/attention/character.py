...
"""Make target labels for the Attention model (CSJ corpus)."""
...
from ..ctc.fix_trans import fix_transcript
from ..ctc.fix_trans import is_hiragana, is_katakana, is_kanji, is_alphabet
...
def load_sdb(label_paths, label_type, run_root_path, is_test=None,
        ...
        for key, row in df.iterrows():
            ...
            if utt_index == utt_index_pre:
                ...
                if left != right:
                    ...
                    # Clean transcript
                    transcript = '<' + \
                        fix_transcript(transcript, speaker_name) + '>'
                    transcript_kanji = '<' + fix_transcript(transcript_kanji,
                                                            speaker_name) + '>'
                    ...
                    transcript = re.sub(r'_>', '>', transcript)
                    transcript_kanji = re.sub(r'<_', '<', transcript_kanji)
                    transcript_kanji = re.sub(r'_>', '>', transcript_kanji)

                    # Skip silence & noise only utterance
                    if transcript not in ['<>', '<_>', '<NZ>']:
    ...
    mapping_file_path = join(run_root_path, 'labels/attention', file_name)
    if save_map_file:
