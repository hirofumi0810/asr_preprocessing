...
def main():
    ...
    save_path_train = mkdir(os.path.join(prep.fbank_path, 'train'))
    save_path_dev = mkdir(os.path.join(prep.fbank_path, 'dev'))
    save_path_test = mkdir(os.path.join(prep.fbank_path, 'test'))
    ...
    with open('wav2fbank_train.scp', 'w') as f:
        for wav_train_path in sorted(wav_train_paths):
            ...
            save_path = os.path.join(
                save_path_train, speaker_name + '_' + wav_index + '.htk')
    ...
    with open('wav2fbank_dev.scp', 'w') as f:
        for wav_dev_path in sorted(wav_dev_paths):
            ...
            save_path = os.path.join(
                save_path_dev, speaker_name + '_' + wav_index + '.htk')
    ...
    with open('wav2fbank_test.scp', 'w') as f:
        for wav_test_path in sorted(wav_test_paths):
            ...
            save_path = os.path.join(
                save_path_test, speaker_name + '_' + wav_index + '.htk')
