...
def main():
    ...
    save_train_fisher_path = mkdir(os.path.join(
        prep.train_data_fisher_path, 'fbank'))
    save_test_path = mkdir(os.path.join(prep.test_data_path, 'fbank/swbd'))
    save_test_callhome_path = mkdir(os.path.join(
        prep.test_data_path, 'fbank/callhome'))
    ...
    # train (LDC97S62)
    ##############################
    wav_paths = [os.path.join(prep.train_data_path, wav_dir)
                 for wav_dir in glob(os.path.join(prep.train_data_path, 'wav/*.wav'))]
    if len(wav_paths) == 0:
    ...
    # train (Fisher)
    ##############################
    wav_paths = [os.path.join(prep.train_data_fisher_path, wav_dir)
                 for wav_dir in glob(os.path.join(prep.train_data_fisher_path, 'wav/*/*.wav'))]
    if len(wav_paths) == 0:
        raise ValueError('Convert from sph to wav files.')
    ...
    with open('wav2fbank_train_fisher.scp', 'w') as f:
        for wav_path in sorted(wav_paths):
            ...
            save_path = os.path.join(
                save_train_fisher_path, number, wav_index + '.htk')
            f.write(wav_path + '  ' + save_path + '\n')

    ##############################
    # test (eval2000, swbd)
    ##############################
    wav_paths = [os.path.join(prep.test_data_path, wav_dir)
                 for wav_dir in glob(os.path.join(prep.test_data_path, 'wav/swbd/*.wav'))]
    if len(wav_paths) == 0:
        raise ValueError('Convert from sph to wav files.')
    ...
    ##############################
    wav_paths = [os.path.join(prep.test_data_path, wav_dir)
                 for wav_dir in glob(os.path.join(prep.test_data_path, 'wav/callhome/*.wav'))]
    if len(wav_paths) == 0:
        raise ValueError('Convert from sph to wav files.')
    elif len(wav_paths) != 40:
        raise ValueError(("File number is not correct ",
    ...
    with open('wav2fbank_test_callhome.scp', 'w') as f:
        for wav_path in sorted(wav_paths):
            wav_index = os.path.basename(wav_path).split('.')[0]
            save_path = os.path.join(
                save_test_callhome_path, wav_index + '.htk')
            f.write(wav_path + '  ' + save_path + '\n')
