...
def read_htk(htk_paths,  speaker_dict, normalize, save_path=None):
    ...
    for htk_path in htk_paths:
        ...
        return_list.append(
            read_each_htk(htk_path, speaker_name, speaker_dict[speaker_name], normalize))
    ...
    if normalize:
        ...
        feature_dim = 123
        train_mean_sum, train_std_sum = np.zeros(
            (feature_dim,)), np.zeros((feature_dim,))
        ...
        if save_path is not None:
            ...
            np.save(os.path.join(statistic_save_path,
                                 'train_mean.npy'), train_mean_global)
            np.save(os.path.join(statistic_save_path,
                                 'train_std.npy'), train_std_global)
...
def read_each_htk(htk_path, speaker_name, utterance_dict, normalize):
    """Read each HTK file.
    Args:
    ...
    if normalize:
        ...
        utterance_dict_sorted = sorted(
            input_data_dict.items(), key=lambda x: x[0])
        for index, (utt_index, utt_info) in enumerate(utterance_dict_sorted):
