...
def main():
    ...
    save_train_path = join(prep.fbank_path, 'train')
    save_train_all_path = join(prep.fbank_path, 'train_all')
    save_eval1_path = join(prep.fbank_path, 'eval1')
    save_eval2_path = join(prep.fbank_path, 'eval2')
    save_eval3_path = join(prep.fbank_path, 'eval3')
    save_dialog_train_path = join(prep.fbank_path, 'dialog_train')
    save_dialog_dev_path = join(prep.fbank_path, 'dialog_dev')
    save_dialog_test_path = join(prep.fbank_path, 'dialog_test')
    ...
    with open('wav2fbank_train_all.scp', 'w') as f:
        for wav_path in wav_train_all_paths:
            ...
            save_path = os.path.join(
                save_train_all_path, speaker_name + '.htk')
            f.write(wav_path + '  ' + save_path + '\n')
    ...
    with open('wav2fbank_dialog_train.scp', 'w') as f:
        for wav_path in wav_dialog_train_paths:
            speaker_name = wav_path.split('/')[-1].split('.')[0]
            save_path = os.path.join(
                save_dialog_train_path, speaker_name + '.htk')
            f.write(wav_path + '  ' + save_path + '\n')
    ...
    with open('wav2fbank_dialog_dev.scp', 'w') as f:
        for wav_path in wav_dialog_dev_paths:
            speaker_name = wav_path.split('/')[-1].split('.')[0]
            save_path = os.path.join(
                save_dialog_dev_path, speaker_name + '.htk')
            f.write(wav_path + '  ' + save_path + '\n')
    ...
    with open('wav2fbank_dialog_test.scp', 'w') as f:
        for wav_path in wav_dialog_test_paths:
            speaker_name = wav_path.split('/')[-1].split('.')[0]
            save_path = os.path.join(
                save_dialog_test_path, speaker_name + '.htk')
            f.write(wav_path + '  ' + save_path + '\n')
