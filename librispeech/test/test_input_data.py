...
class TestInputSpeakerNorm(unittest.TestCase):
    ...
    def test(self):
        ...
        self.htk_save_path = '/n/sd8/inaguma/corpus/librispeech/htk'
        ...
        if tool == 'htk':
            audio_paths = {
                'train': [path for path in sorted(glob(
                    join(self.htk_save_path, 'train_clean100/*/*/*.htk')))],
                'dev_clean': [path for path in sorted(glob(
                    join(self.htk_save_path, 'dev_clean/*/*/*.htk')))],
                'dev_other': [path for path in sorted(glob(
                    join(self.htk_save_path, 'dev_other/*/*/*.htk')))],
                'test_clean': [path for path in sorted(glob(
                    join(self.htk_save_path, 'test_clean/*/*/*.htk')))],
                'test_other': [path for path in sorted(glob(
                    join(self.htk_save_path, 'test_other/*/*/*.htk')))],
            }
        else:
            audio_paths = {
                'train': self.prep.wav(data_type='train'),
                'dev_clean': self.prep.wav(data_type='dev_clean'),
                'dev_other': self.prep.wav(data_type='_otherdev'),
                'test_clean': self.prep.wav(data_type='test_clean'),
                'test_other': self.prep.wav(data_type='test_other')
            }
